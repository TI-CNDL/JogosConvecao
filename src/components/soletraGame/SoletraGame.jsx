...
export default function SoletraGame({
    ...
    const {
        ...
        resetGame,
        buildMaskedWord,
    } = useSoletraGameLogic({ data, settings, onScore, onRoundComplete, onGameOver });
        ...
        <div className="soletra-game panel">
            ...
            <div className="soletra-targets" aria-label="Progresso das palavras">
                {targets.map((target, idx) => {
                    ...
                    return (
                        <div key={`${target.palavra}-${idx}`} className="soletra-target-row">
                            <div
                            ...
                            <div className="soletra-target-hint">
                                {isLocked ? (
                                    <span className="soletra-locked-msg">Resolva a palavra anterior</span>
                                ) : solved ? (
                                    "Palavra encontrada"
                                ) : (
                                    target.dica
            ...
            >
                {Array.from({ length: activeWordLength }).map((_, idx) => {
                    ...
                    const colorClass = lastAttemptColors
                        ...
                        : "";
                    return (
                        <div key={`slot-${idx}`} className={`soletra-input-slot ${colorClass}`}>
                            {letter ?? ""}
                        </div>
                    );
                })}
            ...
            {/* ── Colmeia de letras ── */}
            <div className="soletra-honeycomb" role="group" aria-label="Colmeia de letras">
                <button
